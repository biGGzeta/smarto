...
# -*- coding: utf-8 -*-
"""
Professional Continuous Data Capture System with ML Integration - FIXED VERSION
Sistema profesional con ML adaptativo para clasificación inteligente de patrones - CORREGIDO
Autor: biGGzeta
Fecha: 2025-10-05 16:06:41 UTC
Versión: 4.1.0 (ML Integrated Fixed)
"""
...
class ProfessionalDataCapture:
    """Sistema profesional de captura multi-stream con ML - FIXED VERSION"""
    ...
    def __init__(self, symbol: str = "ETHUSD_PERP"):
        ...
        # WebSocket URLs for multiple streams - CORREGIDO
        self.websocket_urls = {
            "ticker": "wss://dstream.binance.com/ws/ethusd_perp@ticker",
            "orderbook": "wss://dstream.binance.com/ws/ethusd_perp@depth5@100ms",  # 🔧 CORREGIDO: 5 niveles
        ...
        self.config = {
            ...
            "save_interval_seconds": 60,  # 🔧 CORREGIDO: Guardar cada 1 minuto
            "orderbook_levels": 5,        # 🔧 CORREGIDO: 5 niveles para coincidir con URL
        ...
        self.data_source = "binance_professional_ml_fixed"
        self.capture_stats = {
            "ticker_points": 0,
            "orderbook_updates": 0,
            "klines_received": 0,
        ...
        self.logger.info(f"[INIT] Professional Data Capture con ML FIXED iniciado para {symbol}")
        ...
        self.logger.info(f"[FIX] OrderBook URL: {self.websocket_urls['orderbook']}")
    ...
    def setup_logging(self):
        ...
        # File handler
        log_file = os.path.join(log_dir, f"professional_capture_ml_fixed_{self.symbol}_{datetime.datetime.now().strftime('%Y%m%d')}.log")
    ...
    async def start_professional_capture(self):
        """Iniciar captura profesional multi-stream con ML - FIXED"""
        
        self.logger.info("[START] Iniciando captura profesional multi-stream con ML FIXED")
        self.running = True
    ...
    async def _process_orderbook_data(self, data: Dict, timestamp: datetime.datetime):
        """Procesar datos del orderbook profesional - FIXED VERSION"""
        
        try:
            # 🔍 DEBUG CRÍTICO: Ver estructura de data
            self.logger.info(f"[ORDERBOOK-DEBUG] Data keys: {list(data.keys())}")
            if len(str(data)) < 500:
                self.logger.info(f"[ORDERBOOK-DEBUG] Full data: {data}")
            else:
                self.logger.info(f"[ORDERBOOK-DEBUG] Data sample: {str(data)[:300]}...")
            ...
            if 'bids' in data and 'asks' in data:
                bids_data = data['bids']
                asks_data = data['asks']
                self.logger.info("[ORDERBOOK] Using standard format (bids/asks)")
            elif 'b' in data and 'a' in data:
                bids_data = data['b']
                asks_data = data['a']
                self.logger.info("[ORDERBOOK] Using alternative format (b/a)")
            else:
                self.logger.warning(f"[ORDERBOOK] No recognized orderbook format. Available keys: {list(data.keys())}")
                return
            ...
            if not bids or not asks:
                self.logger.warning(f"[ORDERBOOK] Empty bids ({len(bids)}) or asks ({len(asks)})")
                return
            
            # Log successful processing
            self.logger.info(f"[ORDERBOOK] ✅ Processing {len(bids)} bids, {len(asks)} asks")
            ...
            # Log whale detection if found
            if whale_bids > 0 or whale_asks > 0:
                self.logger.info(f"[ORDERBOOK] 🐋 Whale orders found: Bids: ${whale_bids:,.0f}, Asks: ${whale_asks:,.0f}")
            ...
            # Log successful update cada 10 updates
            if self.capture_stats["orderbook_updates"] % 10 == 0:
                self.logger.info(f"[ORDERBOOK] Updates: {self.capture_stats['orderbook_updates']}, "
                               f"Spread: ${real_spread:.2f}, Imbalance: {orderbook_imbalance:+.3f}, "
                               f"Whales: {whale_bids + whale_asks:,.0f}")
            ...
            self.logger.error(f"[ORDERBOOK] Data causing error: {data}")
    
    async def _process_kline_data(self, stream_name: str, data: Dict, timestamp: datetime.datetime):
        """Procesar datos de klines profesionales - FIXED VERSION"""
        
        try:
            # 🔍 DEBUG: Ver estructura de klines
            self.logger.debug(f"[{stream_name.upper()}] Data keys: {list(data.keys())}")
            
            if 'k' not in data:
                self.logger.debug(f"[{stream_name.upper()}] No 'k' key in data: {list(data.keys())}")
                return
            
            kline = data['k']
            
            # 🔧 FIX: Para testing, procesar klines aunque no estén cerradas
            # Comentar esta línea para ver klines en tiempo real durante testing:
            if not kline.get('x', False):  # Solo klines cerradas
                self.logger.debug(f"[{stream_name.upper()}] Kline not closed yet, skipping")
                return
            ...
            # Log kline received
            self.logger.info(f"[{stream_name.upper()}] ✅ Kline received: O:{kline_data['open']:.2f} "
                           f"H:{kline_data['high']:.2f} L:{kline_data['low']:.2f} C:{kline_data['close']:.2f} "
                           f"V:{kline_data['volume']:.0f} B/S:{kline_data['buy_sell_ratio']:.2f}")
    ...
    async def _process_trade_data(self, data: Dict, timestamp: datetime.datetime):
        ...
        try:
            ...
            # Log trade cada 100 trades
            if self.capture_stats["trades_processed"] % 100 == 0:
                self.logger.debug(f"[TRADES] Processed: {self.capture_stats['trades_processed']}, "
                                f"Latest: ${trade_data['price']:.2f} x {trade_data['quantity']:.2f}")
    ...
    async def _process_mark_price_data(self, data: Dict, timestamp: datetime.datetime):
        ...
        try:
            if 'p' in data:
                ...
                # Log mark price cada 50 updates
                if self.capture_stats["ticker_points"] % 50 == 0:
                    self.logger.debug(f"[MARKPRICE] Mark price: ${mark_price:.2f}")
    ...
    async def _enqueue_ticker_data(self, basic_data: Dict):
        ...
        # 🔧 CORREGIDO: Log cada 3 captures (15s) en lugar de 6 (30s)
        if self.capture_stats["ticker_points"] % 3 == 0:
            self.logger.info(f"[PROFESSIONAL] ${professional_point.price:.2f} "
                           ...
                           f"OB Updates: {self.capture_stats['orderbook_updates']} "
    ...
    async def _log_whale_activity(self, orderbook_snapshot: Dict):
        ...
        if whale_bids > whale_asks * 2:
            ...
            self.logger.info(f"[WHALE] 🐋 Whale activity: Bids ${whale_bids:,.0f}, Asks ${whale_asks:,.0f}")
...
async def test_professional_ml_capture():
    """Test del sistema profesional con ML - FIXED VERSION"""
    
    print("🚀 TESTING PROFESSIONAL CONTINUOUS DATA CAPTURE - ML INTEGRATED FIXED")
    ...
    # 🔧 CORREGIDO: Test de 4 minutos para ver ML en acción
    try:
        await asyncio.wait_for(capture.start_professional_capture(), timeout=240)  # 4 minutos
    except asyncio.TimeoutError:
        print("✅ Test completado - sistema profesional con ML FIXED funcionando")
        
        # Mostrar estadísticas finales
        status = capture.get_professional_status()
        print(f"\n📊 RESULTADOS PROFESIONALES CON ML FIXED:")
